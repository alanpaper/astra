...
2. **对话** (`/chat`) — 与 AI 模型对话，支持 @ 项目提及、action 链接触发命令、**代码块一键执行**
...
│   │   ├── command_runner.rs     # Shell 命令执行（流式输出 + 超时控制）
...
| 修改代码块执行逻辑 | `src/routes/chat/MarkdownMessage.svelte` — `handleExecClick` / `autoExecuteActionLinks` / 事件监听 |
| 修改命令执行后端 | `src-tauri/src/command_runner.rs` — `run_command` 函数 |
