...
print(response.choices[0].message.content)`
  ...
  async function loadProvider() {
    ...
    error = '';
    try {
      provider = await invoke<ProviderConfig>('get_provider', { id: providerId });
  ...
  async function selectModel(modelId: string) {
    if (!provider) return;
    try {
      await invoke('set_active_model', { providerId: provider.id, modelId });
  ...
  async function saveEdit() {
    editError = '';
    if (!editName.trim()) { editError = '请输入名称'; return; }
    if (!editBaseUrl.trim()) { editError = '请输入 API 地址'; return; }
  ...
  {#if provider}
    ...
    <div class="info-card">
      <div class="card-header-row">
        ...
        <div class="card-header-actions">
          ...
          <button class="btn-danger-sm" onclick={() => showDeleteConfirm = true}>
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          ...
          <span class="info-value mono highlight">{provider.active_model ?? '未选择'}</span>
        </div>
        <div class="info-item">
          <span class="info-label">创建时间</span>
          <span class="info-value">{formatTime(provider.created_at)}</span>
        </div>
        <div class="info-item">
          <span class="info-label">更新时间</span>
          <span class="info-value">{formatTime(provider.updated_at)}</span>
        </div>
        <div class="info-item">
          <span class="info-label">模型数量</span>
          <span class="info-value">{models.length > 0 ? models.length : '—'}</span>
        </div>
      </div>
    </div>
    ...
    <div class="section-card">
      <div class="section-header">
        ...
        <button class="btn-refresh-models" onclick={fetchModels} disabled={modelsLoading}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class:spinning={modelsLoading}><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>
          刷新模型列表
        </button>
      </div>
      ...
      {#if !modelsLoading && models.length > 0}
        <div class="models-list">
          {#each models as model (model.id)}
            ...
            <div class="model-row" class:active={isActive}>
              <div class="model-row-info">
                <span class="model-row-id">{model.id}</span>
                {#if model.owned_by}
                  <span class="model-row-owner">{model.owned_by}</span>
                {/if}
              </div>
              <div class="model-row-actions">
                ...
                {:else}
                  <button class="btn-select" onclick={() => selectModel(model.id)}>切换</button>
                {/if}
              </div>
            </div>
          {/each}
    ...
    <div class="section-card">
      <div class="section-header">
        ...
        <button class="btn-copy" onclick={() => copyCode(activeTab === 'curl' ? curlExample : pythonExample)}>
          {#if copied}
      ...
      <div class="tabs">
        <button class="tab" class:active={activeTab === 'curl'} onclick={() => activeTab = 'curl'}>cURL</button>
        <button class="tab" class:active={activeTab === 'python'} onclick={() => activeTab = 'python'}>Python</button>
      </div>

      <pre class="code-block"><code>{activeTab === 'curl' ? curlExample : pythonExample}</code></pre>
    </div>
  {/if}
</div>
...
{#if showEditModal}
  ...
  <div class="modal-overlay" onclick={() => showEditModal = false} role="presentation">
    <!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions a11y_roles -->
    <div class="modal" onclick={(e) => e.stopPropagation()} role="dialog" aria-label="编辑接口" tabindex="-1">
      <div class="modal-header">
        <h2>编辑接口</h2>
        <button class="modal-close" onclick={() => showEditModal = false} aria-label="关闭">
...
{#if showDeleteConfirm}
  ...
  <div class="modal-overlay" onclick={() => showDeleteConfirm = false} role="presentation">
    <!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions a11y_roles -->
    <div class="modal modal-sm" onclick={(e) => e.stopPropagation()} role="dialog" aria-label="确认删除" tabindex="-1">
      <div class="modal-header">
        <h2>确认删除</h2>
        <button class="modal-close" onclick={() => showDeleteConfirm = false} aria-label="取消">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
      </div>
      <div class="modal-body">
